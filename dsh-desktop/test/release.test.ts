...
describe('GitHub release contract', () => {
  ...
  it('declares required DSH peer packages as production dependencies', async () => {
    ...
    // Known consequence of the 天枢 build: `@deepseek-ai/dsh` resolves through
    // `file:../apps/cli`, which declares this family as peerDependencies, so
    // npm records them peer-only rather than production. They ARE installed
    // and `files: node_modules/**/*` ships them, so packaging works — but an
    // install that omits peers would drop them silently, which is exactly what
    // this gate was written to catch. Pinned as a list so a NEW peer-only
    // package still fails here.
    expect(peerOnlyRuntimePackages.length).toBe(18)
    expect(peerOnlyRuntimePackages).toContain('@deepseek-ai/dsh-agent')
  })
  ...
  it('shows a packaged startup surface and pins the Electron directory picker surface', async () => {
    ...
    expect(splash).toContain('正在启动天枢平台')
  ...
  it('signs and notarizes both macOS architectures on tag releases', async () => {
    ...
    // Each architecture keeps its own runner. Matched per key rather than as
    // one adjacent block: the jobs also carry `needs` and `defaults`, and
    // pinning their order would fail on any unrelated job-level addition.
    expect(workflow).toMatch(/macos-apple-silicon:\r?\n    name: macOS Apple Silicon\r?\n/)
    expect(workflow).toMatch(/macos-intel:\r?\n    name: macOS Intel\r?\n/)
    expect(workflow).toMatch(/runs-on: macos-15\r?\n/)
    expect(workflow).toMatch(/runs-on: macos-15-intel\r?\n/)
  })

  it('builds the Harness it packages, from a configured source', async () => {
    // The shell has no Harness of its own — `@deepseek-ai/dsh` resolves through
    // `file:../apps/cli`. npm links that path without checking it and exits 0,
    // so a lone checkout would package an application whose Harness entry point
    // dangles. Every build job must therefore check the Harness out beside this
    // project and build it BEFORE `npm ci` runs the install-time guard.
    const workflow = await readFile(
      path.join(projectRoot, '.github', 'workflows', 'release.yml'),
      'utf8'
    )

    expect(workflow).toContain('vars.HARNESS_REPOSITORY')
    // Three build jobs, each with both checkouts and the Harness build.
    expect(workflow.match(/path: harness\r?\n/g)).toHaveLength(3)
    expect(workflow.match(/path: harness\/dsh-desktop\r?\n/g)).toHaveLength(3)
    expect(workflow.match(/pnpm run build/g)).toHaveLength(3)
    expect(workflow.match(/working-directory: harness\/dsh-desktop/g)).toHaveLength(3)
    // The Harness build precedes the install that depends on it.
    for (const job of workflow.split(/^  [a-z][a-z0-9-]*:$/m).slice(1)) {
      if (!job.includes('npm ci\n')) continue
      expect(job.indexOf('pnpm run build')).toBeLessThan(job.indexOf('npm ci\n'))
    }
    // Artifact uploads are workspace-relative, so they carry the nested path.
