  ...
  switch (command) {
    ...
    case 'about':
      await dialog.showMessageBox(window, {
        type: 'info',
        title: '天枢平台',
        message: `天枢平台 ${app.getVersion()}`,
        detail: harnessLocale() === 'zh'
          ? '天枢平台桌面客户端。'
          : 'The 天枢平台 desktop client.',
        buttons: [harnessLocale() === 'zh' ? '好' : 'OK'],
        noLink: true
      })
      break
    case 'quit':
...
function showUnexpectedError(error: unknown): void {
  ...
  dialog.showErrorBox('天枢平台 发生错误', message)
}
...
function installMenu(): void {
  // The user's Harness language preference, not the raw OS locale: every
  // other surface here reads it, and the menu sitting in a different language
  // than the window it belongs to is the visible bug.
  const isChinese = harnessLocale() === 'zh'
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    { label: isChinese ? '窗口' : 'Window', submenu: [{ role: 'minimize' }, { role: 'close' }] }
  ]
