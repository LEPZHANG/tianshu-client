...
describe('SettingsPanel chrome seats', () => {
  it('escapes the trigger subtree so a restyled sidebar cannot recolor the panel', () => {
    // The trigger seats in the sidebar foot. A brand column that rebinds the
    // ink scale for its own subtree (light text on a saturated fill) would
    // otherwise hand the panel white text on its white surface.
    const { view } = mount()
    openPanel()
