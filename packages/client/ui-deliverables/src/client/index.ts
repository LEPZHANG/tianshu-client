...
export function apply(ctx: ClientContext): void {
  ...
  const mentions: ChatFileMentions = {
    forClosing(owner) {
      // Not gated on produced files the way the turn-tail chain entry is: the
      // resolver's second tier links paths the turn only cited, and a turn
      // that explains without writing anything is exactly when that tier is
      // the whole value. An empty list simply skips the first tier.
