...
describe('producedFileMentions resolver', () => {
  ...
  it('resolves exact paths and unique basenames; ambiguity and unknowns stay unresolved', () => {
    ...
    // A basename two paths share stays unresolved by the produced tier rather
    // than guessing, and falls through to the path-spelling tier below.
    expect(resolver.resolve('style.css')?.title).toBe('style.css')
    expect(resolver.resolve('notes.md')?.title).toBe('notes.md')
    expect(basename('a\\b\\c.txt')).toBe('c.txt')
  })

  it('links paths the turn only cited, so explanation gets the same affordance as production', () => {
    const opened: string[] = []
    const resolver = producedFileMentions([], (path) => { opened.push(path) }, label)
    for (const [token, expected] of [
      ...
      ['dist/', 'dist/'],
      ['README.md', 'README.md'],
      // A line reference identifies a place in the file, not a different file.
      ...
      expect(resolver.resolve(token)?.title, token).toBe(expected)
    }
    resolver.resolve('src/app.ts:42')?.open()
    expect(opened).toEqual(['src/app.ts'])
  })

  it('leaves prose that is not a path inert', () => {
    const resolver = producedFileMentions([], () => {}, label)
    for (const token of [
      // Inline code without a separator, path prefix, or extension is far more
      // often a command, package, or symbol — linking those would make
      // ordinary prose a field of dead links.
      'npm', 'useState', 'SIGKILL',
      // A trailing segment too long to be an extension: 17 characters, one
      // past the cap.
      'file.abcdefghijklmnopq',
      '', '   ', 'multi\nline', 'trailing\r',
      // URL schemes address something other than a local file; `javascript:`
      // must never reach an opener.
      'https://example.com/a.txt', 'http://x.dev', 'data:text/plain,hi',
      'javascript:alert(1)', 'JavaScript:alert(1)',
    ]) {
      expect(resolver.resolve(token), token).toBeUndefined()
    }
  })

  it('links a dotted identifier that is spelled like a filename', () => {
    // The known false positive of the bare-filename tier: spelling alone
    // cannot separate `Object.prototype.hasOwnProperty` from `archive.tar.gz`.
    // Accepted rather than narrowed, because the cost is one click that opens
    // nothing (the opener swallows Host/OS failures) while narrowing would
    // drop real multi-part filenames.
    const resolver = producedFileMentions([], () => {}, label)
    expect(resolver.resolve('Object.prototype.hasOwnProperty')?.title)
      .toBe('Object.prototype.hasOwnProperty')
    expect(resolver.resolve('archive.tar.gz')?.title).toBe('archive.tar.gz')
  })
})
...
describe('plugin registration', () => {
  it('registers the tail entry and fiber disposal removes it', async () => {
    ...
    // A turn that produced nothing still yields the vocabulary: its second
    // tier links paths the turn only cited, which is the whole value for a
    // turn that explains without writing anything.
    const citedOnly = service?.forClosing(tailOwner(undefined, 2))
    expect(citedOnly?.resolve('src/app.ts')?.title).toBe('src/app.ts')
    expect(citedOnly?.resolve('npm')).toBeUndefined()
