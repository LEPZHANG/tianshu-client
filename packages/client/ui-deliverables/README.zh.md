...
收尾正文以两层承载同一份词表，两层的置信度不同。本插件提供供 chat 视图按收尾消息查询的 `chatFileMentions` 服务。`producedFileMentions` 先针对本轮产出路径解析行内代码 token——按精确路径，或当 token 恰好等于某条产出路径的 basename 且这样的路径仅有一条时；两条产出路径共享同一 basename 时，文本保持不可点击而不作猜测，因此产出文件的提及永远不会打开错误的文件。其余 token 落入路径拼写层：它接受以 `/`、`~/`、`./`、`../`、盘符或 UNC 根开头的 token、任何包含分隔符的 token，以及带有可信扩展名的裸文件名，并会去掉结尾的 `:行号`、`:行号:列号` 或 `#L行号` 引用。该层链接的是本轮仅提及的文件：文件可能并不存在，打开失败也会被静默吞掉——这是让「解释」获得与「产出」同等可点击性的代价。裸标识符与 URL scheme 保持不可点击。解析成功的提及保留代码标签，并采用 Markdown 样式表的链接样式：静止时为链接蓝色，悬停时显示下划线，与 URL 提升的行内代码完全一致——完整路径作为其 `title`；提及绝不会渲染在链接内部或流式文本中。决策记录：[行内文件提及 Agent Note](../../../.agents/notes/implemented/feature/2026-08-07-web-inline-file-mentions.md)。
...
- **产出文件匹配只认精确路径或唯一 basename。**后缀式提及（`out/index.html` 写作 `index.html` 可解析；`deep/out/index.html` 写作 `out/index.html` 则不行）无法命中该层；等真实的收尾消息形态产生需求后再放宽匹配规则。这类 token 仍会落入路径拼写层，由后者在不知文件是否存在的前提下将其链接。
- **路径拼写层判断的是拼写，不是存在性。**它无法区分带点的标识符与多段文件名（`Object.prototype.hasOwnProperty` 与 `archive.tar.gz` 会被同等链接），也会链接从未被写入的路径。两者的代价都只是一次打不开任何东西的点击，因为打开器会吞掉 Host／OS 失败。收紧规则会漏掉真实文件名，因此这里宁可接受误报也不放过链接；真正的修复需要该界面并不具备的 Host 侧存在性检查。
