...
describe('query rail', () => {
  // Captured once, before any spy exists: this suite's afterEach does not
  // restore mocks, so re-reading the prototype per test would capture the
  // previous spy and chain them into unbounded recursion. Called with an
  // explicit receiver below, so the unbound reference is deliberate.
  // oxlint-disable-next-line typescript/unbound-method -- rebound with .call(this) at every use
  const realRect: (this: Element) => DOMRect = Element.prototype.getBoundingClientRect

  afterEach(() => { vi.restoreAllMocks() })

  /**
   * The rail measures real boxes during its first layout effect, and jsdom
   * reports zeros — so the geometry must exist before render, not after.
   * Only the scroll host is given a box; everything else keeps jsdom's zeros.
   */
  function installRailGeometry(reading = { top: 0, bottom: 800 }): void {
    // Patched on Element, where jsdom defines it — a spy on HTMLElement would
    // shadow it without ever being reached.
    vi.spyOn(Element.prototype, 'getBoundingClientRect').mockImplementation(function (
      this: Element,
    ) {
      if (!this.className.includes('scroll')) return realRect.call(this)
      return {
        top: reading.top, bottom: reading.bottom, left: 40, right: 840,
        width: 800, height: reading.bottom - reading.top, x: 40, y: reading.top,
        toJSON: () => ({}),
      } satisfies DOMRect
    })
  }

  it('places one tick per query and none for a single-query conversation', () => {
    // One query is the whole conversation: a rail would be chrome with
    // nothing to navigate between.
    installRailGeometry()
    const single = makeHarness({ nodes: [user(1, 'only ask'), assistant(2, 'a')] })
    const singleView = render(<single.ChatView {...single.props} />)
    expect(singleView.container.querySelector('[data-query-rail]')).toBeNull()

    const h = makeHarness({
      nodes: [user(1, 'first ask'), assistant(2, 'a'), user(3, 'second ask')],
    })
    const view = render(<h.ChatView {...h.props} />)
    const rail = view.getByLabelText('Query 导航')
    const ticks = within(rail).getAllByRole('button')
    expect(ticks).toHaveLength(2)
    // Assistant replies are not navigable points: the reader navigates by
    // what they asked.
    expect(ticks.every(tick => tick.dataset.queryKey !== undefined)).toBe(true)
  })

  it('previews a query on hover and labels each tick for assistive tech', () => {
    installRailGeometry()
    const h = makeHarness({
      nodes: [user(1, 'build\n\n  the   thing'), assistant(2, 'a'), user(3, 'now ship it')],
    })
    const view = render(<h.ChatView {...h.props} />)
    const rail = view.getByLabelText('Query 导航')
    const [first] = within(rail).getAllByRole('button')
    // Whitespace is collapsed: the tooltip clamps to three lines, and the
    // query's own newlines would spend them all on blank space.
    expect(first?.getAttribute('aria-label')).toBe('Query 1：build the thing')
    expect(within(rail).queryByRole('tooltip')).toBeNull()
    fireEvent.mouseEnter(first as HTMLElement)
    const tooltip = within(rail).getByRole('tooltip')
    expect(tooltip.textContent).toBe('Query 1build the thing')
    fireEvent.mouseLeave(rail)
    expect(within(rail).queryByRole('tooltip')).toBeNull()
  })

  it('names a query that carries no text by what it does carry', () => {
    installRailGeometry()
    const imageQuery: UserMessageNode = {
      kind: 'user', seq: 1, time: 1_000, source: null,
      content: [{ type: 'image', attachment: { id: 'a1' } }] as never,
    }
    const emptyQuery: UserMessageNode = {
      kind: 'user', seq: 3, time: 3_000, source: null, content: [] as never,
    }
    const h = makeHarness({ nodes: [imageQuery, assistant(2, 'a'), emptyQuery] })
    const view = render(<h.ChatView {...h.props} />)
    const rail = view.getByLabelText('Query 导航')
    const labels = within(rail).getAllByRole('button').map(tick => tick.getAttribute('aria-label'))
    expect(labels).toEqual(['Query 1：图片 Query', 'Query 2：无文本 Query'])
  })

  it('scrolls to a query and marks it without waiting for the scroll to settle', () => {
    installRailGeometry()
    // jsdom has matchMedia but no media state; this is the ordinary case.
    ...
    const rail = view.getByLabelText('Query 导航')
    const [, second] = within(rail).getAllByRole('button')
    fireEvent.click(second as HTMLElement)
    expect(scrollTo).toHaveBeenCalledTimes(1)
    // The click is the reader's statement of where they are, so the mark
    // moves now rather than after the smooth scroll lands.
    expect(second?.getAttribute('aria-current')).toBe('step')
  })

  it('honors reduced motion when jumping between queries', () => {
    installRailGeometry()
    ...
    scroller.scrollTo = scrollTo
    const rail = view.getByLabelText('Query 导航')
    fireEvent.click(within(rail).getAllByRole('button')[1] as HTMLElement)
    expect(scrollTo).toHaveBeenCalledWith(expect.objectContaining({ behavior: 'auto' }))
  })

  it('withdraws the rail when the reading area is too short to hold it', () => {
    // A 100px reading area leaves no room for a rail of ticks.
    installRailGeometry({ top: 0, bottom: 100 })
    const h = makeHarness({
      nodes: [user(1, 'first ask'), assistant(2, 'a'), user(3, 'second ask')],
    })
    const view = render(<h.ChatView {...h.props} />)
    expect(view.container.querySelector('[data-query-rail]')).toBeNull()
  })
})
