...
聊天视图带有一条 query 导航条：位于正文旁的固定刻度列，每条持久用户 query 与 steering 消息各占一格——也就是读者心中「我问过什么」的那些位置。它标记当前阅读位置所属的 query，判定阈值取可见正文向下 55%（下限 72px），使刚滚入视野的 query 不会从读者仍在阅读的那一轮手中抢走标记；点击刻度会滚动到该 query 并立即标记，而不等平滑滚动结束，并遵循 `prefers-reduced-motion`。悬停或聚焦会展开三行预览，其文本已折叠空白并截断至 240 字符。该导航条测量已解析的 scrollport 与 composer 顶边，而不是在 CSS 中重述这些几何量，因此它会跟随窗口、侧边栏与不断增高的输入框；当阅读区不足 144px、视口窄于 980px，或会话中不足两条 query 时，它会整体撤下。
