...
import { PendingSteeringBubble, contentParts } from './MessageItem.tsx'
import { ChatNodeSeat } from './ChatNodeSeat.tsx'
import { QueryRail, queryAtViewport, type RailQuery } from './QueryRail.tsx'
import { formatRunDuration } from './message-chrome.ts'
import css from './ChatView.module.css'

const FOLLOW_THRESHOLD = 24

/** Tooltip previews are clamped here, not by CSS alone: a pasted wall of text
 *  would otherwise be carried in full through every render. */
const PREVIEW_LIMIT = 240

/** Clearance above a query row after the rail scrolls to it. */
const QUERY_SCROLL_MARGIN = 24
...
export function ChatView({
  ...
  // The rail's navigable set: durable user turns and the steering messages
  // that interrupt them — the points a reader thinks of as "what I asked".
  const queries = useMemo<RailQuery[]>(() => order.flatMap((key) => {
    const node = nodeStore.get(key)
    // `data` is `unknown` on the base view node, because ChatNodeDataMap is
    // merge-extensible and this build may not know every registered kind. The
    // two kinds read here are declared by this package's own message module.
    ...
    // Collapsed to one line: the tooltip clamps to three lines, and a query's
    // own newlines would spend them all on blank space.
    const text = content.text.replace(/\s+/g, ' ').trim()
    const fallback = content.images.length > 0 ? t('chat.queryImageOnly') : t('chat.queryNoText')
    return [{ key, preview: (text === '' ? fallback : text).slice(0, PREVIEW_LIMIT) }]
  }), [order, nodeStore, t])
  ...
  /** Mirrors the state so scroll handling can compare without a re-render. */
  ...
  const syncActiveQuery = (local: HTMLElement, el: HTMLElement): void => {
  ...
  const toBottom = (el: HTMLElement): void => {
    ...
    /* v8 ignore next -- ref-null guard: every caller resolved the scrollport from this ref. */
  ...
  // New queries arriving, or history opening, move which turn owns the
  // reading position without any scroll event firing.
  useLayoutEffect(() => {
    const local = listRef.current
    /* v8 ignore next -- ref-null guard: layout effects run after the list node commits. */
  ...
  const scrollToQuery = (key: string): void => {
    const local = listRef.current
    /* v8 ignore next -- ref-null guard: the rail only renders once the list has committed. */
    if (local === null) return
    const row = anchorElement(local, key)
    /* v8 ignore next -- the rail's keys come from the rendered order, so the row exists. */
    if (row === null) return
    const el = scrollerOf(local)
    const reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches
    el.scrollTo({
      top: Math.max(0, el.scrollTop + flowTop(row, el) - QUERY_SCROLL_MARGIN),
      behavior: reducedMotion ? 'auto' : 'smooth',
    })
    // Mark immediately rather than waiting for the smooth scroll to settle:
    // the click is the reader's statement of where they are.
  ...
  return (
    <div className={css.root}>
      ...
      {/* Last in DOM order so its layout effect runs after the list and column
          refs have attached (React commits child effects first); the rail is
          `position: fixed`, so document order does not place it. */}
      <QueryRail
        queries={queries}
