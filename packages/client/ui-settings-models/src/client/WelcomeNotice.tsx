...
/**
 * Whether a save failed because the caller is not authenticated.
 *
 * A deployment may put this UI behind a login (the transport answers `401`
 * once the session lapses). "Please try again" is wrong there — retrying
 * fails identically until the user signs in — so the copy has to differ.
 * Matched on the transport's rendered status because that is all the failure
 * carries: `rpc.ts` throws `transport failure … HTTP <status>` with no code.
 * @param message - the failure text the store captured.
 * @returns true when the message reports an unauthorized transport status.
 */
      ...
      {state.error === null
        ...
        : (
          <p className={css.error} role="alert">
            {t(isUnauthorized(state.error) ? 'welcomeErrorUnauthorized' : 'welcomeError')}
          </p>
        )}
