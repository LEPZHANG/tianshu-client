...
describe('ModelsSection', () => {
  ...
  it('adds a dormant provider with a derived reference and stores its key', async () => {
    ...
    fireEvent.click(screen.getByText(en.add))
    // The add card opens on a chosen route, summarized rather than in the grid:
    // the common path is filling in that route's key, not picking another.
    const field = await screen.findByLabelText(en.provider)
    expect(within(field).getAllByText('anthropic').length).toBeGreaterThan(0)
    fireEvent.click(within(field).getByText(en.changeProvider))
    const grid = await screen.findByLabelText(en.provider)
    // Every addable route is offered, each naming its id beside its display
    // name — here the fixture makes them the same string.
    expect(within(grid).getAllByRole('button').map(card => card.textContent))
      .toEqual(['anthropicanthropic', 'brokenbroken', 'plainplain'])
  ...
  it('switches the add card target and degrades unknown or broken targets loudly', async () => {
    ...
    const field = await screen.findByLabelText(en.provider)
    fireEvent.click(within(field).getByText(en.changeProvider))
    // Cards are indexed rather than named: this fixture's display names equal
    // their route ids, so each card's text repeats the same string.
    fireEvent.click(within(await screen.findByLabelText(en.provider)).getAllByRole('button')[1] as HTMLElement)
    await screen.findByText(/unresolvable settings path/)
    fireEvent.click(within(await screen.findByLabelText(en.provider)).getByText(en.changeProvider))
    fireEvent.click(within(await screen.findByLabelText(en.provider)).getAllByRole('button')[2] as HTMLElement)
    await waitFor(() => {
