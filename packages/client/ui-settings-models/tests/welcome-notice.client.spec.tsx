...
describe('WelcomeNotice', () => {
  ...
  it('names a lapsed session instead of asking for a retry that cannot work', async () => {
    // A deployment may put this UI behind a login. Once the session lapses the
    // transport answers 401 and every retry fails identically, so the generic
    // "please try again" sends the user in a loop. Only the rendered status is
    // available to distinguish it: rpc.ts throws `transport failure … HTTP 401`
    // and carries no code.
    let resolveWrite: (value: unknown) => void = () => {}
    const write = new Promise<unknown>((resolve) => { resolveWrite = resolve })
    const h = mount(undefined, () => write)
    await screen.findByRole('dialog')
    fireEvent.click(screen.getByRole('button', { name: WELCOME_NOTICE_COPY.zh.continueLabel }))
    resolveWrite(Promise.reject(new Error('transport failure for /api/settings.mutate: HTTP 401')))
    expect((await screen.findByRole('alert')).textContent).toBe(zh.welcomeErrorUnauthorized)
    expect(h.complete).not.toHaveBeenCalled()
  })
})
