...
function mountColumn(): { column: HTMLElement; quiet: () => boolean } {
  // The shell reads only the nav selection from its store; a real instance is
  // the sanctioned zero-machinery path.
  const nav = createNavStore().create()
  const useStore = ((selector: (s: unknown) => unknown) =>
    selector(nav.getSnapshot())) as TianshuSidebarComponentProps['useStore']
  const view = render(
    <TianshuSidebar
      ...
      renderSlot={((_key: string, owner: OwnerOf<'sidebar.workspaces'>) =>
        <div data-testid="region" data-wide={owner.wide} />) as TianshuSidebarComponentProps['renderSlot']}
    />,
  )
...
describe('TianshuSidebar pointer-revealed scrollbars', () => {
