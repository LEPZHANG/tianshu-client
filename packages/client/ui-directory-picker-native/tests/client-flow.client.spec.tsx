...
describe('directory-picker-native client half', () => {
  it('drives the host service when no embedding shell publishes a bridge', async () => {
    const b = await bench()
    b.declare()
    await b.ctx.plugin({ inject: [...inject], apply }).await()
    const injected = (b.slots.entries('sidebar.workspaces.directoryFlow')[0]
      ?.inject as () => { pick: () => Promise<string | null> })()
    await expect(injected.pick()).resolves.toBe('/tmp/picked')
    expect(b.pickDirectory).toHaveBeenCalledOnce()
  })

  it('prefers an embedding shell\'s directory bridge over the host service', async () => {
    // A desktop shell runs its platform dialog and disables the host picker, so
    // when it publishes a bridge that bridge is the only working chooser.
    const bridge = { pick: vi.fn(async (): Promise<string | null> => '/from/shell') }
    ;(globalThis as { dshDesktopDirectoryPicker?: unknown }).dshDesktopDirectoryPicker = bridge
    try {
      const b = await bench()
      b.declare()
      await b.ctx.plugin({ inject: [...inject], apply }).await()
      const injected = (b.slots.entries('sidebar.workspaces.directoryFlow')[0]
        ?.inject as () => { pick: () => Promise<string | null> })()
      await expect(injected.pick()).resolves.toBe('/from/shell')
      expect(bridge.pick).toHaveBeenCalledOnce()
      expect(b.pickDirectory).not.toHaveBeenCalled()
    } finally {
      delete (globalThis as { dshDesktopDirectoryPicker?: unknown }).dshDesktopDirectoryPicker
    }
  })
