...
export function httpErrorCode(status: number, error?: WireError['error']): string {
  ...
  // Quota is classified before status, because providers report an exhausted
  // balance as 403 as readily as 429. Reading the status first would file those
  // under AUTH and tell the user to fix a key that is working.
  if (isQuotaExceededError(detail)) return QUOTA_EXCEEDED_CODE
  if (status === 401) return 'AUTH'
  // 403 is a usable credential the provider refused for this request (region,
  // model entitlement, org policy). Kept apart from AUTH so the diagnosis does
  // not blame the key.
