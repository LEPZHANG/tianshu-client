...
describe('DeepSeekAdapter against a mock server', () => {
  ...
  it('reads quota wording ahead of the status that carried it', () => {
    // Providers return an exhausted balance as 403 as readily as 429. Status
    // first would file it under AUTH and send the user to replace a key that
    // works.
    expect(httpErrorCode(403, { code: 'insufficient_quota', message: 'insufficient balance' }))
      .toBe(QUOTA_EXCEEDED_CODE)
    expect(httpErrorCode(401, { message: 'exceeded your current quota' })).toBe(QUOTA_EXCEEDED_CODE)
  })

  it('separates a refused request from an unusable credential', () => {
    // 403 without quota wording is a working key the provider refused for this
    // request — region, model entitlement, org policy — not a bad key.
    expect(httpErrorCode(403, { message: 'model not available in your region' })).toBe('FORBIDDEN')
    expect(httpErrorCode(403)).toBe('FORBIDDEN')
