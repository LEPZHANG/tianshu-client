...
describe('mapStopReason / mapUsage', () => {
  ...
  it('maps routable HTTP-ish error messages to stable codes', () => {
    ...
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'HTTP 403: region not enabled' })))
  ...
  it('reads quota wording ahead of the status that carried it', () => {
    // Providers report an exhausted balance as 403 as readily as 429. Matching
    // the status first would file it under AUTH and send the user to replace a
    // key that works.
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'HTTP 403: insufficient balance' })))
      .toMatchObject({ kind: 'error', failure: { code: 'QUOTA' } })
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'HTTP 401: quota exceeded' })))
      .toMatchObject({ kind: 'error', failure: { code: 'QUOTA' } })
  })
