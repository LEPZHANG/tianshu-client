...
export class ApiProxyService extends Service implements ApiProxy {
  // `directoryPicker` is deliberately absent: a composition may serve directory
  // selection outside the harness (the desktop shell uses Electron's native
  // dialog and disables the Koffi-based host worker). The three methods that
  // need it read it through `ctx.get` and answer `directory-picker-unavailable`
  // when nothing provides it, so an absent picker degrades those three calls
  // instead of preventing the whole gateway from activating.
  static inject = [
